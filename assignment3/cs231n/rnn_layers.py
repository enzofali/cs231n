...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Compute the next hidden state using the tanh activation function
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0 # Set the initial hidden state
    
    # Iterate over each timestep
    for t in range(T):
        next_h, step_cache = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b) # Compute the forward pass
        h[:, t, :] = next_h # Store the computed hidden state in the h matrix
        grad_cache.append(step_cache) # Store the cache values
        prev_h = next_h # Update the previous hidden state
...
def rnn_backward(dh, cache):
    ...
    # Initialize the previous hidden state gradient
    dprev_h = np.zeros((N, H))

    # Iterate over each timestep
    for t in reversed(range(T)):
        # Compute the backward pass
        dx[:, t, :], dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:, t, :] + dprev_h, grad_cache[t])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
...
def word_embedding_forward(x, W):
    ...
    # Perform the forward pass for word embeddings using array indexing
    out = W[x, :]  # Select the word vectors corresponding to the input indices
...
def word_embedding_backward(dout, cache):
    ...
    # Perform the backward pass for word embeddings using np.add.at
    np.add.at(dW, x, dout)
