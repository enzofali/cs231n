...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # l(k, k+N)
        l_k_kN = -torch.log(torch.exp(sim(z_k, z_k_N)) / sum(torch.exp(sim(z_k, out[i]) / tau) for i in range(2 * N) if i != k))
        # l(k+N, k)
        l_kN_k = -torch.log(torch.exp(sim(z_k_N, z_k)) / sum(torch.exp(sim(z_k_N, out[i]) / tau) for i in range(2 * N) if i != k + N))
        # total loss
...
def sim_positive_pairs(out_left, out_right):
    ...
    # Normalize output of the left branch
    norm_left = out_left / torch.linalg.norm(out_left, dim=1, keepdim=True)
    # Normalize output of the right branch
    ...
    pos_pairs = torch.sum(torch.mul(norm_left, norm_right), dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    # Normalize output of projection head
    out_norm = out / torch.linalg.norm(out, dim=1, keepdim=True)

    # Compute the dot product between all pairs of augmented examples in the batch
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Duplicate pos_pairs to match the shape of exponential
    pos_pairs = pos_pairs.repeat(2, 1) 
