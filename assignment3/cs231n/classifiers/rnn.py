...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # Initial hidden state for the image features
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            # Embedding layer
            emb_word, _ = word_embedding_forward(word, W_embed)
            # RNN step
            h, _ = rnn_step_forward(emb_word, h0, Wx, Wh, b)
            # Affine transformation
            scores, _ = temporal_affine_forward(h, W_vocab, b_vocab)
            # Select the word with the highest score
            word = np.argmax(scores, axis=1)
            # Write the word index to the appropriate slot in the captions variable
            captions[:, i] = word
